...
def compile_formula(formula, verbose=False):
    ...
    groups = [list(x) for x in re.findall("^[ ]*([A-Z])|[+*=][ ]*([A-Z])", formula)]
    first = filter(lambda x: x!='', sum(groups, []))
#    print '---',first, '|',  formula
    parms = ', '.join(letters)
    tokens = map(compile_word, re.split('([A-Z]+)', formula))
    body = ''.join(tokens)
    false = 'False if any([x==0 for x in [%s]]) else' % (','.join(first))
    f = 'lambda %s: %s (%s)' % (parms, false, body)
    if verbose: print f
