...
def poly(coefs):
    ...
    def _x(x, k=coefs):
        if k == ():
            ...
            return k[0] + x*_x(x, k[1:])
    _x.coefs = coefs
    N = [(str(x) if i == 0 else
          str(x) + ' * x' if i == 1 and x > 1 else
          'x' if i == 1 and x == 1 else
          str(x) + ' * x**' + str(i) if x>1 else
          'x**' + str(i) )
         for i,x in enumerate(coefs)
         if x > 0 ]
    _x.__name__ = ' + '.join (reversed( N ))
    _x.poly = True
    return _x
...
def is_poly(x):
    ...
    return getattr(x, 'poly', False)
    

def add(p1, p2):
    "Return a new polynomial which is the sum of polynomials p1 and p2."
    print 'add', p1.coefs, p2.coefs
    coefs = [0] * max (len (p1.coefs), len(p2.coefs))
    print map(sum, zip(p1.coefs, coefs))
    for i,a in enumerate (p1.coefs): coefs[i] += a
    for i,b in enumerate (p2.coefs): coefs[i] += b
    print ':', coefs
...
#    print 'sub', p1,p2, p1.coefs, p2.coefs
    return add(p1, poly(tuple([-x for x in p2.coefs])))


def mul(p1, p2):
    "Return a new polynomial which is the product of polynomials p1 and p2."
#    print 'mul', p1,p2, p1.coefs, p2.coefs
    coefs = [0] * (len(p1.coefs + p2.coefs)-1)
...
#    print '\n\n\nderiv', p.coefs
    return  poly(tuple([i*x for i,x in enumerate(p.coefs) if i>0]))
...
#    print '\n\n\nint', p.coefs
    ...
    return poly( tuple([0] + [x/(i+1) for i,x in enumerate (p.coefs)]) )
