...
def do(action, state):
    ...
    return (
        (score + yard, 0 , cards[1:] ) if action == 'gather' else
        (score, 0, cards[1:]) if cards [0] == 'F' else
        (score, 1+yard, cards[1:])
    )
...
def superior(A, B=take5):
    ...
    a = average_score(A) - average_score(B)
    print a
    return a > 1.5
...
def strategy(state):
    (score, yard, cards) = state
    # your code here
    return ('wait' if len (cards) == 0 or yard < 20 else
            'gather'
    )
