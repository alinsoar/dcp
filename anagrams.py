...
def anagrams(phrase, shortest=2):
    ...
    def getlist (letters):
        ...
        t =  sorted (filter (lambda x: len(x) >= shortest, words), key = len)
        return t if set(''.join(t)) == set (letters) else []
    # your code here
    phrase = phrase.replace (' ', '')
    words = getlist(phrase)
    print ':', words
    queue = map(lambda(x): [(x,), removed(phrase, x)], words)
    res = set ()
    while (queue):
        comb, letters = queue.pop(0)
        last = comb[-1]
        #print '<-', comb, '__', letters
        if letters == '':
            #print ' '.join (comb)
            res.add (' '.join (comb))
        rem = find_words (letters)
        f = filter (lambda x : x > last, rem)
        for x in ([comb + (new,), removed(letters, new)] for new in f):
            queue.append (x)
    return res
