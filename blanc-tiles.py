...
POINTS = dict(A=1, B=3, C=3, D=2, E=1, F=4, G=2, H=4, I=1, J=8, K=5,
              L=1, M=3, N=1, O=1, P=3, Q=10, R=1, S=1, T=1, U=1, V=4, W=4, X=8, Y=4,
              Z=10, _=0)
...
def removed(letters, remove):
    ...
    #print letters, remove
    letters2 = letters
    for L in remove:
        if letters2.count(L) > 0:
            letters2 = letters2.replace(L, '', 1)
        else:
            letters2 = letters2.replace('_', '', 1)
    #print letters2, letters, remove
    assert len(letters2) + len (remove) == len (letters)
    return letters2
...
LOWER = map (str.lower, LETTERS)
for x in LOWER: POINTS [x] = 0
            ...
            for L in hand:
                ...
                if '_' in hand:
                    for x in possibilities:
                        add_suffixes(hand.replace('_', '', 1), pre+x.lower(), start, row, results)
    return results
...
def find_prefixes(hand, pre='', results=None):
    ...
    if pre.upper() in WORDS or pre in PREFIXES: results.add(pre)
    if pre.upper() in PREFIXES:
        for L in hand:
            if L == '_':
                for x in POINTS:
                    find_prefixes(hand.replace('_', '', 1), pre+(x.lower()), results)
            else:
    ...
    for (i, sq) in enumerate(row[1:-1], 1):
        if isinstance(sq, set):
            ...
            if pre: ## Add to the letters already on the board
                ...
                #if '_' in hand: print '+++',  hand , find_prefixes(hand)
