...
def solve_parking_puzzle(start, N=N):
    ...
    def apply(state, action):
        dup = dict(state)
        car, pos = action
        dup[car] = [x+pos for x in state[car]]
        return tuple([(x,tuple(v)) for x,v in dup.items()])

    def succ(state):
        state = dict(state)
        a = reduce(lambda x,y: x+y, [x for x in state.values()])
        empty = [ x
                  for x in range (N**2)
                  if x not in a ]
        res = ()
        out = state['@'][0]
        for x,pos in state.items():
            if x == '@': continue
            speed = pos[1]-pos[0]
            p = speed
            while (pos[0] - p in empty):
                res += ((x, -p),)
                p += speed
            p = speed
            while (pos[-1] + p in empty or x=='*' and pos[-1] + p == out):
                res += ((x, p),)
                p += speed
        return dict(map(lambda x: (apply(state, x), x), res))
    ...
    def okay(state):
        ...
        car = state['*'][0]+1
        return car  == out
    return shortest_path_search (start, succ, okay)
...
def grid(cars, N=N):
    ...
    for car, pos in cars:
        if car == '*':
            ...
            break
    border = sorted([x
                     for x in (locs(0, N-1, 1) +
                               locs(N, N-1, N) +
                               locs (N-1, N, N) +
                               locs (N*(N-1), N-1, 1))
                     if x != out[0]])
    return cars + (('@', out), ('|', tuple(border)))

def show(m, state, N=N):
...
show ('init', puzzle0)
sol = solve_parking_puzzle (puzzle0)
print path_actions(sol)
show('0',sol[-1])


show ('init', puzzle1)
sol = solve_parking_puzzle (puzzle1)
print path_actions(sol)
show('1', sol[-1])

show ('init', puzzle2)
sol = solve_parking_puzzle (puzzle2)
print path_actions(sol)
show('2', sol[-1])

show ('init', puzzle3)
sol = solve_parking_puzzle (puzzle3)
print path_actions(sol)
show('3', sol[-1])
