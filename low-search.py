...
def lowest_cost_search(start, successors, is_goal, action_cost):
    ...
    while frontier:
        path = frontier.pop (0)
        laststate = final_state (path)
        if is_goal (laststate): return path
        explored.add (laststate)
        pcost = action_cost (path)
        for (state, action) in successors (state).items ():
            if state not in explored:
                cost = pcost + action_cost (action)
                p2 = path + [(action, cost), state]
                add_to_frontier (frontier, p2)
    return Fail
