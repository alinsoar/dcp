...
def double_out(total):
    ...
    # your code here
    target = {'SB': 25, 'DB': 50, 'OFF': 0}
    target = dict (target, **dict( [(m+str(i), i * x)
                                 for i in range (1, 21)
                                    for m,x in {'S': 1, 'D': 2, 'T': 3}.items()]))
    S = sorted (
        [ [e for e in [x, y, z] if e != 'OFF']
          for x,a in target.items()
          if (a < total or
              a == total and x[0] == 'D')
          for y,b in target.items()
          if (b <= a and
              (a + b < total or
               a + b == total and y[0] == 'D' or y == 'OFF'))
          for z,c in target.items()
          if (c <= b and
               a+b+c == total and (z[0] == 'D' or z == 'OFF'))
        ], key = len )
    if not S:
        return None
    else:
...
def ring_outcome(target, miss):
    ...
    print
    print 'ring:', target, miss, dict(D+R)

def section_outcome(target, miss):
    n = target[1:] if target != 'OFF' else target
    K = neighbour[n]
    print
    print ('section:', target, miss,
           dict ([(n, 1-miss)] + [(k, miss/len(K)) for k in K]))

#print section_outcome('T20', .2)
ring_outcome('T20', .1)
ring_outcome('S', .2)
#ring_outcome('SB', .2)
#section_outcome('SB', .2)
# print ring_outcome('DB', .05)
#ring_outcome('DB', .2)
# print ring_outcome('SB', .2)
#section_outcome('DB', .2)
