...
def bsuccessors3(state):
    ...
    if light is 0:
        return dict(  [ ((here - set([x, y]), there | set([x, y]), 1), (set([x,y]), '->'))
                        for x in here for y in here ])
    else:
        return dict( [ ((here | set([x, y]), there - set([x,y]), 0), (set([x, y]), '<-'))
                       for x in there for y in there ])
