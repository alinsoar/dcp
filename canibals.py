...
def csuccessors(state):
    ...
    res = {}
    if B1 > 0:
        if M1 >= 1: res[ (M1-1, C1, B1-1, M2+1, C2, B2+1) ] = "M->"
        if C1 >= 1: res[ (M1, C1-1, B1-1, M2, C2+1, B2+1) ] = "C->"
        if M1 >= 2: res[ (M1-2, C1, B1-1, M2+2, C2, B2+1) ] = "MM->"
        if C1 >= 2: res[ (M1, C1-2, B1-1, M2, C2+2, B2+1) ] = "CC->"
        if M1 > 0 and C1 > 0: res[ (M1-1, C1-1, B1-1, M2+1, C2+1, B2+1) ] = "MC->"
    if B2 > 0:
        if M2 >= 1: res[ (M1+1, C1, B1+1, M2-1, C2, B2-1) ] = "<-M"
        if C2 >= 1: res[ (M1, C1+1, B1+1, M2, C2-1, B2-1) ] = "<-C"
        if M2 >= 2: res[ (M1+2, C1, B1+1, M2-2, C2, B2-1) ] = "<-MM"
        if C2 >= 2: res[ (M1, C1+2, B1+1, M2, C2-2, B2-1) ] = "<-CC"
        if M2 > 0 and C2 > 0: res[ (M1+1, C1+1, B1+1, M2-1, C2-1, B2-1) ] = "<-MC"
    for x in res: print '::', x, res[x]
    return res
